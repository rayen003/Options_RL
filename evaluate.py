...
REGIME_SCENARIOS = {"bull": 1, "bear": -1}
METRICS = (
    "terminal_return_pct",
    "max_drawdown_pct",
    "spread_paid_dollars",
...
def run_policy_episode(
    ...
    episode_length=60,
    training_seed=None,
    regime_scenario=None,
):
    """Run one policy on one reproducible simulator path."""
    if episode_length <= 0:
        raise ValueError("episode_length must be positive")
    if regime_scenario is not None and regime_scenario not in REGIME_SCENARIOS:
        raise ValueError(f"Unknown regime scenario: {regime_scenario}")
    env_options = {"regime_switch_prob": 0.0} if regime_scenario is not None else {}
    env = OptionsEnv(episode_length=episode_length, **env_options)
    reset_options = (
        {"initial_regime": REGIME_SCENARIOS[regime_scenario]}
        if regime_scenario is not None
        else None
    )
...
def run_regime_study(seeds=range(20_000, 20_020), models=None, episode_length=60):
    """Run all policies on paired paths in fixed bull and bear scenarios."""
    models = models or {}
    seeds = tuple(seeds)
    rows = []
...
def _trajectory_rows(episodes):
    """Expand per-episode paths to one row per marked portfolio value."""
    for episode in episodes:
        initial_value = float(episode["portfolio_values"][0])
        for step, portfolio_value in enumerate(episode["portfolio_values"]):
            yield {
                "regime_scenario": episode["regime_scenario"],
                "policy": episode["policy"],
                "training_seed": episode["training_seed"],
                "market_seed": episode["seed"],
                "step": step,
                "regime": episode["regimes"][step],
                "spot": episode["spots"][step],
                "portfolio_value": portfolio_value,
                "cumulative_pnl_dollars": portfolio_value - initial_value,
                "cumulative_pnl_pct": 100 * (portfolio_value / initial_value - 1),
                "action": "" if step == 0 else episode["actions"][step - 1],
            }


def write_regime_report(episodes, output_dir, episode_length=60, model_paths=None):
    """Save step-level paths, controlled-regime plot, and reproduction manifest."""
    if not episodes:
        raise ValueError("Cannot report zero regime episodes")
    scenarios = {episode["regime_scenario"] for episode in episodes}
    if scenarios != set(REGIME_SCENARIOS):
        raise ValueError("Regime report requires both bull and bear scenario episodes")

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    targets = [output_dir / "regime_trajectories.csv", output_dir / "regime_pnl.png", output_dir / "manifest.json"]
    if any(target.exists() for target in targets):
        raise FileExistsError(f"Regime report already exists in {output_dir}")

    trajectories = list(_trajectory_rows(episodes))
    trajectory_columns = (
        "regime_scenario", "policy", "training_seed", "market_seed", "step", "regime",
        "spot", "portfolio_value", "cumulative_pnl_dollars", "cumulative_pnl_pct", "action",
    )
    _write_csv(targets[0], trajectories, trajectory_columns)
    ...
    for axis, scenario in zip(axes, ("bull", "bear")):
        ...
        for (row_scenario, policy, training_seed, _), path in grouped.items():
            if row_scenario == scenario:
                policy_groups[(policy, training_seed)].append(path)
        for key, paths in sorted(
            policy_groups.items(),
            key=lambda item: (item[0][0], item[0][1] if item[0][1] is not None else -1),
        ):
            curves = np.asarray(
                [[float(point["cumulative_pnl_dollars"]) for point in path] for path in paths]
            )
    ...
    manifest = {
        "protocol": "docs/REGIME_STUDY_PROTOCOL.md",
        "git_commit": revision.stdout.strip() if revision.returncode == 0 else None,
        "evaluation_seeds": sorted({int(episode["seed"]) for episode in episodes}),
        "training_seeds": sorted({int(episode["training_seed"]) for episode in episodes if episode["training_seed"] is not None}),
        "episode_length": episode_length,
        "regime_scenarios": REGIME_SCENARIOS,
        "model_paths": {str(seed): str(path) for seed, path in (model_paths or {}).items()},
        "warning": "Controlled synthetic scenarios; not evidence of real-market performance.",
    }
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help="Run fixed bull/bear trajectory study only (use fresh seed range, e.g. --seed-start 20000)",
    )
    ...
    if args.regime_only:
        rows = run_regime_study(seeds=seeds, models=models, episode_length=args.episode_length)
        write_regime_report(rows, args.output, episode_length=args.episode_length, model_paths=model_paths)
        print("Controlled bull/bear simulator study; not real-market performance.")
        for scenario in REGIME_SCENARIOS:
            scenario_rows = [row for row in rows if row["regime_scenario"] == scenario]
            print(f"{scenario.capitalize()} regime: {len(scenario_rows)} paired policy episodes")
    else:
